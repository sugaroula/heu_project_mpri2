...
def test_CatSo_var_MaxCut_bench(restart_rate, mean_budget_restart, var_budget_restart, budget):
    ...
    best_found_list = []
    print("=" * 20)
    print(f"{restart_rate}, {mean_budget_restart}, {var_budget_restart}, {budget}")
    for i in range(20):
...
class CatSo_pop2_bias_rate_plot:
    ...
    def __call__(self, problem: ioh.problem.IntegerSingleObjective) -> None:
        ...
        if result_backup >= result_fav: # backup is better than fav, we swap
            copy = favorite
            favorite = backup
            backup = copy
            result_copy = result_fav
